...
)
{
    ...
    do
    {
        ...
        Platform_MemorySet(pSessionHandle, 0x00, sizeof(TSS_TPMI_SH_AUTH_SESSION));
        Platform_MemorySet(pNonceTPM, 0x00, sizeof(TSS_TPM2B_NONCE));
        // Marshal the request
        pbBuffer = rgbRequest;
        ...
        if (RC_SUCCESS != unReturnValue)
            break;
        unReturnValue = TSS_TPM2B_NONCE_Marshal(pNonceCaller, &pbBuffer, &nSizeRemaining);
        if (RC_SUCCESS != unReturnValue)
            break;
        unReturnValue = TSS_TPM2B_ENCRYPTED_SECRET_Marshal(pEncryptedSalt, &pbBuffer, &nSizeRemaining);
        if (RC_SUCCESS != unReturnValue)
            break;
        unReturnValue = TSS_TPM_SE_Marshal(&sessionType, &pbBuffer, &nSizeRemaining);
        if (RC_SUCCESS != unReturnValue)
            break;
        unReturnValue = TSS_TPMT_SYM_DEF_Marshal(pSymmetric, &pbBuffer, &nSizeRemaining);
        if (RC_SUCCESS != unReturnValue)
            break;
