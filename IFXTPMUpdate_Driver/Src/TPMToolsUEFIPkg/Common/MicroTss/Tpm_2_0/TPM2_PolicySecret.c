...
)
{
    ...
    do
    {
        ...
        Platform_MemorySet(pTimeout, 0x00, sizeof(TSS_TPM2B_TIMEOUT));
        Platform_MemorySet(pPolicyTicket, 0x00, sizeof(TSS_TPMT_TK_AUTH));
        Platform_MemorySet(pAuthHandleSessionResponseData, 0x00, sizeof(TSS_AcknowledgmentResponseData));
        // Marshal the request
        pbBuffer = rgbRequest;
        ...
        // Marshal Session Context
        {
            ...
            if (RC_SUCCESS != unReturnValue)
                break;
            unReturnValue = TSS_AuthorizationCommandData_Marshal(pAuthHandleSessionRequestData, &pbBuffer, &nSizeRemaining);
            if (RC_SUCCESS != unReturnValue)
                break;
        ...
        unReturnValue = TSS_TPM2B_NONCE_Marshal(pNonceTPM, &pbBuffer, &nSizeRemaining);
        if (RC_SUCCESS != unReturnValue)
            break;
        unReturnValue = TSS_TPM2B_DIGEST_Marshal(pCpHashA, &pbBuffer, &nSizeRemaining);
        if (RC_SUCCESS != unReturnValue)
            break;
        unReturnValue = TSS_TPM2B_NONCE_Marshal(pPolicyRef, &pbBuffer, &nSizeRemaining);
        if (RC_SUCCESS != unReturnValue)
            break;
